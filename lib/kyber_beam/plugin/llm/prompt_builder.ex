defmodule Kyber.Plugin.LLM.PromptBuilder do
  ...
  def build_system_prompt(_chat_id) do
    # Step 1: Load SOUL.md — prefer vault (Knowledge resolves layout), fall back to file
    soul_content =
      case safe_knowledge_call({:get_tiered, "SOUL.md", :l2}) do
        {:ok, %{body: body}} when is_binary(body) and body != "" ->
          body

        _ ->
          # Try legacy path through Knowledge
    ...
    long_term_memory =
      case safe_knowledge_call({:get_tiered, "MEMORY.md", :l2}) do
        {:ok, %{body: body}} when is_binary(body) and body != "" ->
        ...
        _ ->
          ...
          case safe_knowledge_call({:get_tiered, "identity/MEMORY.md", :l2}) do
            ...
            _ ->
              # File fallback: try multi-agent path first, then legacy
              memory_md_from_file()
          end
      end
...
"""
  ...
  defp load_soul_from_file do
    agent = agent_name_for_file_fallback()
    vault = Path.expand("~/.kyber/vault")

    paths = [
      # Multi-agent layout
      Path.join([vault, "agents", agent, "SOUL.md"]),
      # Legacy layout
      Path.join(vault, "identity/SOUL.md"),
      # Priv fallback
  ...
  defp memory_md_from_file do
    agent = agent_name_for_file_fallback()
    vault = Path.expand("~/.kyber/vault")

    paths = [
      Path.join([vault, "agents", agent, "MEMORY.md"]),
      Path.join(vault, "identity/MEMORY.md")
    ]
  ...
  defp agent_name_for_file_fallback do
    if Process.whereis(Kyber.Knowledge) do
      try do
        Kyber.Knowledge.agent_name()
      catch
        :exit, _ -> Kyber.Config.get(:agent_name, "stilgar")
      end
    else
